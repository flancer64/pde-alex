 ...
 * @param {'previous'|'v3'|'legacy'} variant
...
/**
 * Keep source index/constraint names separate from target DEM names. PostgreSQL
 * preserves them on table rename; SQLite has no portable index rename, so its
 * non-constraint source indexes are dropped while source rows remain intact.
 * PostgreSQL identifiers are limited to 63 bytes, so fresh backup names use
 * short hashes and already isolated names are left unchanged.
 * @param {any} connection
 * @param {readonly string[]} sourceNames
 ...
 * @param {any} deps.sourceCompilation
 * @param {'previous'|'v3'|'legacy'} deps.variant
 * @returns {Promise<object>}
 */
...
/**
 * @param {object} deps
 * @param {any} deps.history
 * @param {any} deps.compilation
 * @param {any} deps.connection
 * @returns {Promise<object>}
 */
async function completeHistory({history, compilation, connection}) {
    ...
    if (!attempt) {
        try {
            ...
            // @teqfw/db 2.3 with PostgreSQL inserts the row but returns a Result object;
            // recover the started attempt instead of losing the completed rebuild.
    ...
    constructor({config, connection, connectionFactory, compile, rebuild, history, schemaProvider}) {
        ...
        this.execute = async function () {
            ...
            try {
                ...
                const evidence = await rebuild.exec({mode: 'parallel', compilation: target, sourceCompilation: selected.candidate, source: sourceConnection, target: connection, sourceId: `pde-alex:${selected.variant}:source`, targetId: 'pde-alex:runtime:target', snapshot, transformations});
